...
SQL_USER="ff_app"
TEMP_DATABASE="ff_release_verify"
...
TEMP_DATABASE_SECRET="ff-release-verify-database-url"
TEMP_CORS_SECRET="ff-release-verify-cors-origins"
...
--plan is read-only and reports inventory and proposed disposable resources.
--apply captures Render once and performs two restore/release/verification passes.
--cleanup removes only the fixed FF-58 disposable resources.
EOF
}
...
parse_args() {
  ...
  [[ -n "$MODE" ]] || die "choose --plan, --apply, or --cleanup"
}
...
assert_safe_names() {
  [[ "$TEMP_DATABASE" == "ff_release_verify" ]]
  [[ "$TEMP_DATABASE" != "$PERMANENT_DATABASE" ]]
  [[ "$API_SERVICE" == *-verify && "$API_SERVICE" != "$PERMANENT_API_SERVICE" ]]
  [[ "$WEB_SERVICE" == *-verify && "$WEB_SERVICE" != "$PERMANENT_WEB_SERVICE" ]]
  [[ "$RELEASE_JOB" == *-verify ]]
  [[ "$TEMP_DATABASE_SECRET" == ff-release-verify-* ]]
  [[ "$TEMP_CORS_SECRET" == ff-release-verify-* ]]
}

disposable_inventory() {
  local found=0
  if resource_exists sql databases describe "$TEMP_DATABASE" \
      --instance "$SQL_INSTANCE" --project "$PROJECT_ID"; then
    log "disposable database exists: ${TEMP_DATABASE}"
    found=1
  fi
  local secret
  for secret in "$TEMP_DATABASE_SECRET" "$TEMP_CORS_SECRET"; do
    if resource_exists secrets describe "$secret" --project "$PROJECT_ID"; then
      log "disposable secret exists: ${secret}"
      found=1
    fi
  done
  if resource_exists run jobs describe "$RELEASE_JOB" \
      --project "$PROJECT_ID" --region "$REGION"; then
    log "disposable job exists: ${RELEASE_JOB}"
    found=1
  fi
  local service
  for service in "$API_SERVICE" "$WEB_SERVICE"; do
    if resource_exists run services describe "$service" \
        --project "$PROJECT_ID" --region "$REGION"; then
      log "disposable service exists: ${service}"
      found=1
    fi
  done
  return "$found"
...
print_plan() {
  ...
  log "target Cloud SQL: ${SQL_INSTANCE}/${TEMP_DATABASE} (disposable)"
...
remove_secret_access() {
  local secret="$1"
  gcloud_cmd secrets remove-iam-policy-binding "$secret" \
    --project "$PROJECT_ID" \
...
cleanup_resources() {
  ...
  unset CSQL_PROXY_TOKEN PGPASSWORD DATABASE_URL
  gcloud_cmd run services delete "$API_SERVICE" --project "$PROJECT_ID" \
    --region "$REGION" --quiet >/dev/null 2>&1
  gcloud_cmd run services delete "$WEB_SERVICE" --project "$PROJECT_ID" \
    --region "$REGION" --quiet >/dev/null 2>&1
  gcloud_cmd run jobs delete "$RELEASE_JOB" --project "$PROJECT_ID" \
    --region "$REGION" --quiet >/dev/null 2>&1
  remove_secret_access "$TEMP_DATABASE_SECRET"
  remove_secret_access "$TEMP_CORS_SECRET"
  gcloud_cmd secrets delete "$TEMP_DATABASE_SECRET" \
    --project "$PROJECT_ID" --quiet >/dev/null 2>&1
  gcloud_cmd secrets delete "$TEMP_CORS_SECRET" \
    --project "$PROJECT_ID" --quiet >/dev/null 2>&1
  gcloud_cmd sql databases delete "$TEMP_DATABASE" \
    --instance "$SQL_INSTANCE" --project "$PROJECT_ID" \
    --quiet >/dev/null 2>&1
  gcloud_cmd run services remove-iam-policy-binding "$API_SERVICE" \
    --project "$PROJECT_ID" --region "$REGION" \
    --member "serviceAccount:${RUNTIME_SERVICE_ACCOUNT}" \
    --role roles/run.invoker --quiet >/dev/null 2>&1
  gcloud_cmd run services remove-iam-policy-binding "$WEB_SERVICE" \
    --project "$PROJECT_ID" --region "$REGION" \
    --member "serviceAccount:${RUNTIME_SERVICE_ACCOUNT}" \
    --role roles/run.invoker --quiet >/dev/null 2>&1
...
cleanup_trap() {
  local status=$?
  if [[ "$APPLY_STARTED" == "true" && "$CLEANUP_COMPLETE" != "true" ]]; then
    cleanup_resources
  fi
...
create_database() {
  gcloud_cmd sql databases create "$TEMP_DATABASE" \
    --instance "$SQL_INSTANCE" --project "$PROJECT_ID" \
    --charset UTF8 --quiet >/dev/null
}
...
capture_restored_counts() {
  ...
  psql --no-psqlrc --set ON_ERROR_STOP=1 --host 127.0.0.1 \
    --port "$PROXY_PORT" --username "$SQL_USER" --dbname "$TEMP_DATABASE" \
...
deploy_release_job() {
  gcloud_cmd run jobs deploy "$RELEASE_JOB" --project "$PROJECT_ID" \
    ...
    --set-secrets "DATABASE_URL=${TEMP_DATABASE_SECRET}:latest,ROOT_ADMIN_USERNAME=ff-root-admin-username:latest" \
...
run_invariants() {
  ...
  DATABASE_URL="postgresql://${SQL_USER}:${ENCODED_DB_PASSWORD}@127.0.0.1:${PROXY_PORT}/${TEMP_DATABASE}?schema=public" \
...
rehearsal_pass() {
  ...
  pg_restore --no-owner --no-acl --exit-on-error --single-transaction \
    ...
    --dbname "$TEMP_DATABASE" "$source_dir/production.dump" >/dev/null ||
...
compare_invariant_reports() {
  python3 - "$SCRATCH_DIR/invariants-1.json" \
    "$SCRATCH_DIR/invariants-2.json" <<'PY'
import json
import sys
...
def stable(path):
    ...
    report.pop("checkedAt", None)
    return report

if stable(sys.argv[1]) != stable(sys.argv[2]):
    raise SystemExit("sanitized invariant reports differ between passes")
PY
}
...
deploy_and_smoke() {
  ...
  gcloud_cmd run deploy "$WEB_SERVICE" --project "$PROJECT_ID" \
    ...
    --min 0 --max 3 --no-allow-unauthenticated --quiet >/dev/null
  local web_url
  web_url="$(
  ...
  create_secret "$TEMP_CORS_SECRET" "$web_url"
  gcloud_cmd run deploy "$API_SERVICE" --project "$PROJECT_ID" \
    --region "$REGION" --image "$API_IMAGE" \
    --service-account "$RUNTIME_SERVICE_ACCOUNT" \
    --set-cloudsql-instances "$CLOUD_SQL_CONNECTION" \
    --set-secrets "DATABASE_URL=${TEMP_DATABASE_SECRET}:latest,JWT_SECRET=ff-jwt-secret:latest,REGISTRATION_INVITE_CODE=ff-registration-invite-code:latest,CORS_ORIGINS=${TEMP_CORS_SECRET}:latest" \
    --set-env-vars "NODE_ENV=production,JWT_EXPIRES_IN=8h,GCP_PROJECT_ID=${PROJECT_ID},GCS_PUBLIC_BUCKET=${PROJECT_ID}-ff-verify-public-images,GCS_QR_BUCKET=${PROJECT_ID}-ff-verify-payment-qr,GCS_SIGNED_URL_TTL_SECONDS=900,LEGACY_SUPABASE_PUBLIC_BUCKET=ff-public-images" \
    --port 8080 --min 0 --max 3 --no-allow-unauthenticated \
    --quiet >/dev/null
  local api_url
  api_url="$(
...
write_evidence() {
  local slower_pass="$PASS_ONE_SECONDS"
  ((PASS_TWO_SECONDS > slower_pass)) && slower_pass="$PASS_TWO_SECONDS"
  local projected="$((CAPTURE_SECONDS + slower_pass + DEPLOY_SMOKE_SECONDS))"
  ((projected <= MAX_PROJECTED_SECONDS)) ||
    die "projected cutover ${projected}s exceeds ${MAX_PROJECTED_SECONDS}s"
  local evidence="$OUTPUT_DIR/ff-58-rehearsal-evidence.json"
...
report = {
    ...
    "releaseExecutions": [
        ...
        os.environ["RELEASE_EXECUTION_TWO"],
    ],
    "timingsSeconds": {
        "capture": int(os.environ["CAPTURE_SECONDS"]),
        "passOne": int(os.environ["PASS_ONE_SECONDS"]),
        "rollbackReset": int(os.environ["RESET_SECONDS"]),
        "passTwo": int(os.environ["PASS_TWO_SECONDS"]),
...
verify_cleanup_and_boundaries() {
  if ! disposable_inventory; then
    die "one or more disposable resources remain after cleanup"
  fi
  [[ "$(permanent_state)" == "$PERMANENT_STATE_BEFORE" ]] ||
    die "permanent database or Cloud Run placeholder changed"
  [[ "$(wif_state)" == "$WIF_STATE_BEFORE" ]] ||
    die "durable WIF trust changed"
  local runtime_policy
  runtime_policy="$(runtime_policy_state)"
  [[ "$runtime_policy" == "$ORIGINAL_RUNTIME_POLICY" ]] ||
    die "runtime service-account IAM policy was not restored"
  log "cleanup and permanent-boundary verification passed"
}
...
apply_rehearsal() {
  ...
  socket_database_url="postgresql://${SQL_USER}:${ENCODED_DB_PASSWORD}@localhost/${TEMP_DATABASE}?host=/cloudsql/${CLOUD_SQL_CONNECTION}"
  create_secret "$TEMP_DATABASE_SECRET" "$socket_database_url"
  ...
  delete_database
  create_database
  RESET_SECONDS="$(( $(date +%s) - reset_started ))"
  rehearsal_pass 2
  compare_invariant_reports
  deploy_and_smoke
  cleanup_resources
  ...
  rm -rf "$SCRATCH_DIR"
  SCRATCH_DIR=""
  log "live two-pass rehearsal completed without permanent changes"
}
