...
# shellcheck source=scripts/rehearse-gcp-migration.sh
source "$script_dir/rehearse-gcp-migration.sh"

fail() {
  printf 'FF-58 focused test failed: %s\n' "$*" >&2
  exit 1
}
...
resource_exists() {
  [[ "$*" == "sql databases describe ff_release_verify --instance ff-restaurent-db --project ff-restaurent" ]]
}
expect_failure "partial disposable resources" disposable_inventory
unset -f resource_exists
...
[[ -f "$HOME/.docker/config.json" ]] ||
  fail "cleanup removed the operator-owned Docker configuration"
grep -Fq "$TEMP_DATABASE" "$cleanup_log"
grep -Fq "$TEMP_DATABASE_SECRET" "$cleanup_log"
grep -Fq "$TEMP_CORS_SECRET" "$cleanup_log"
grep -Fq "$RELEASE_JOB" "$cleanup_log"
grep -Fq "$API_SERVICE" "$cleanup_log"
grep -Fq "$WEB_SERVICE" "$cleanup_log"
if grep -Eq 'databases delete ff_restaurent([[:space:]]|$)' "$cleanup_log"; then
  fail "cleanup targeted the permanent database"
fi
if grep -Eq 'services delete ff-restaurent-(api|web)([[:space:]]|$)' \
    "$cleanup_log"; then
  fail "cleanup targeted a permanent Cloud Run service"
fi

if grep -Eq 'set -x|BACKUP_PASSPHRASE=.*echo|PGPASSWORD=.*echo' \
    "$script_dir/rehearse-gcp-migration.sh"; then
  fail "operator script contains unsafe tracing or secret output"
fi

echo "FF-58 rehearsal focused tests passed"
